        ...
        if (response.length) {
          ...
          mongoClient.connect(url, function (error, database) {
            ...
            dbo.collection(collection_name).updateOne(listingQuery, updates, {upsert: true}, function (error, response) {
              if (error) throw error;
              console.log('System is inserted or updated: ' + JSON.stringify(response));
              res.jsonp(response);
              database.close();
            });
          });
      ...
      if (error) {
        ...
        if (response.length) {
          ...
          mongoClient.connect(url, function (error, database) {
            ...
            dbo.collection(collection_name).updateOne(listingQuery, updates, {upsert: true}, function (error, response) {
              if (error) throw error;
              console.log('System is updated: ' + JSON.stringify(response));
              res.jsonp(response);
              database.close();
            });
          });
...
app.listen(port, env.SERVER_NAME, function () {
  console.log('System service is listening on port ' + port + '!')
});
