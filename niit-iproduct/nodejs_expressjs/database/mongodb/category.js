...
app.post('/' + collection_name + '/add', function (req, res) {
  mongoClient.connect(url, function (error, database) {
    if (error) throw error;
    const dbo = database.db(dbname);
    dbo.collection(collection_auth_name).find({ _id: new ObjectId(req.body.user_id), access_token: req.headers.authorization.split(' ')[1] }).toArray(function (error, response) {
