    ...
    dbo.collection(collection_auth_name).find({
      ...
      if (error) {
        ...
        if (response.length) {
          ...
          dbo.collection('systems').findOne({ type: req.body.system_type }, function (error, response) {
            if (error) throw error;
            ...
            if (response) {
              ...
              mongoClient.connect(url, function (error, database) {
                if (error) throw error;
                const dbo = database.db(dbname);
                dbo.collection('activities').updateOne(listingQueryAction, updateActions, { upsert: true }, function (error, response) {
                  if (error) throw error;
